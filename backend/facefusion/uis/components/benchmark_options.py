...
from facefusion.utils.common_helper import calculate_int_step
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = facefusion.utils.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = facefusion.utils.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(facefusion.utils.choices.benchmark_cycle_count_range),
		minimum = facefusion.utils.choices.benchmark_cycle_count_range[0],
		maximum = facefusion.utils.choices.benchmark_cycle_count_range[-1]
	)
