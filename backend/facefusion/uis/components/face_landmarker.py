...
from facefusion.face import face_landmarker
from facefusion.localization import translator
from facefusion.utils.common_helper import calculate_float_step
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = facefusion.utils.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(facefusion.utils.choices.face_landmarker_score_range),
		minimum = facefusion.utils.choices.face_landmarker_score_range[0],
		maximum = facefusion.utils.choices.face_landmarker_score_range[-1]
	)
