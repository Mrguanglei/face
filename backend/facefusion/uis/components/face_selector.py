...
from facefusion.media.vision import fit_cover_frame, read_static_image, read_video_frame
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = facefusion.utils.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ (FACE_SELECTOR_ORDER_LABELS.get(choice, choice), choice) for choice in facefusion.utils.choices.face_selector_orders ],
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ (FACE_SELECTOR_GENDER_LABELS.get(choice, choice), choice) for choice in [ 'none' ] + facefusion.utils.choices.face_selector_genders ],
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = [ (FACE_SELECTOR_RACE_LABELS.get(choice, choice), choice) for choice in [ 'none' ] + facefusion.utils.choices.face_selector_races ],
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or facefusion.utils.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or facefusion.utils.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = translator.get('uis.face_selector_age_range_slider'),
				minimum = facefusion.utils.choices.face_selector_age_range[0],
				maximum = facefusion.utils.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calculate_int_step(facefusion.utils.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = translator.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calculate_float_step(facefusion.utils.choices.reference_face_distance_range),
		minimum = facefusion.utils.choices.reference_face_distance_range[0],
		maximum = facefusion.utils.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
